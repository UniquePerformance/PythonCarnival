...
@Date: 2020-02-29 07:12:44 -0800
@LastEditTime: 2020-02-29 07:12:44 -0800
@FilePath: /bing/bing/db.py
...
class DBError(Error):
    def __init__(self, message: str):
        if re.search("database is locked", str(message), re.IGNORECASE):
            msg.warn("Database is locked, the request has been stacked onto the request handler.")
...
class db(object):
    ...
    def __init__(
        self, path, auto_commit: bool = True, reconnects: int = 3, auto_connect=False, timeout=5
    ):
        self.path = path
    ...
    def _create_connection(self):
        ...
        try:
            ...
            msg.info(f"Successfully connected to {self.path}")
    ...
    def _attempt_reconnect(self, error_on_fail=False):
        for i in range(self.reconnects):
            msg.info("Attempting to reconnect to %s on attempt %s" % (self.path, i + 1))
            ...
            try:
                ...
                msg.info("Reconnected %s after %s attempts." % (self.path, i + 1))

                break

            except sqlite3.DatabaseError:
                msg.info("Failed to connect to %s on attempt %s" % (self.path, i + 1))
                ...
                if i == self.reconnects - 1:
                    ...
                    if error_on_fail:
                        msg.info(
                            "Connection to the %s failed %s times." % (self.path, self.reconnects)
                        )
    ...
    def create_table(self, table_name: str, values: list, types: list):
        ...
        if len(values) != len(types):
            ...
            return DBError(
                "Insufficient number of %s." % ("values" if len(values) < len(types) else "types")
            )
        ...
        for x in types:
            ...
            if x.upper() not in (
                ...
                return DBError(
                    "Incorrect type provided. '%s' is not a valid data type." % x.capitalize()
                )
        ...
        try:
            msg.info(
                f"CREATE TABLE IF NOT EXISTS {table_name} ({', '.join(reversed([x.upper() for x in values]))})"
            )
            msg.info(values)
    ...
    def update(self, table: str, column, value, column_w: str = None, value_w: str = None):
        ...
        try:
            if column_w is not None and value_w is not None:
                self._handler.execute(
                    f"UPDATE {table} SET {column}=? WHERE {column_w}=?", (value, value_w)
                )
    ...
    def select(
        ...
        try:
            if column_w is not None and value_w is not None:
                if not fetchall:
                    data = self._handler.execute(
                        f"SELECT {', '.join(select)} FROM {table} WHERE {column_w}=? {random}{limit}",
                        (value_w,),
                    ).fetchone()
            ...
            if column_w is None and value_w is None:
                if not fetchall:
                    data = self._handler.execute(
                        f"SELECT {', '.join(select)} FROM {table}{random}{limit}"
                    ).fetchone()
            ...
            if re.search("database is locked", str(e), re.IGNORECASE):
                self._handle_locked(
                    self.select, table, select, column_w, value_w, fetchall, limit, random
                )
